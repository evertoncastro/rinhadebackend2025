...
summary-test:
	...
	@curl -s "http://localhost:9999/payments-summary?from=2000-01-01T00:00:00&to=2900-01-01T00:00:00" | jq '.' || echo "❌ Erro na consulta"


admin-summary-test: ## Testar endpoint /admin/payments-summary nos processadores externos
	@echo "📊 Testando processador padrão"
	@curl -s "http://localhost:8001/admin/payments-summary?from=2000-01-01T00:00:00&to=2900-01-01T00:00:00" --header 'X-Rinha-Token: 123' | jq '.' || echo "❌ Processador padrão não está respondendo"
	@echo ""
	@echo "📊 Testando processador de fallback"
	@curl -s "http://localhost:8002/admin/payments-summary?from=2000-01-01T00:00:00&to=2900-01-01T00:00:00" --header 'X-Rinha-Token: 123' | jq '.' || echo "❌ Processador de fallback não está respondendo"
